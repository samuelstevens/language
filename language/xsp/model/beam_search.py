...
def _compute_batch_indices(batch_size, beam_size):
...
def compute_topk_scores_and_seq(
    sequences, scores, scores_to_gather, flags, beam_size, batch_size
):
  ...
  Args:
    ...
    scores_to_gather: Tensor of scores for each sequence in sequences.
      ...
      Scores to gather is different from scores because for _grow_alive, we will
      need to return log_probs, while for _grow_finished, we will need to return
  ...
  """
    _, topk_indexes = tf.nn.top_k(scores, beam_size)
    ...
    # batch the beam item is in. This will create the i of the i,j coordinate
    # needed for the gather
    batch_pos = _compute_batch_indices(batch_size, beam_size)
...
def _grow_finished(
  ...
  """
    ...
    curr_scores = _apply_negative_infinity_mask(
        curr_scores, tf.logical_not(curr_finished)
    )
...
def _grow_alive(
    curr_seq, curr_scores, curr_log_probs, curr_finished, batch_size, beam_size
):
    """Given sequences and scores, will gather the top k=beam size sequences.
  ...
  """
    ...
    return compute_topk_scores_and_seq(
        curr_seq, curr_scores, curr_log_probs, curr_finished, beam_size, batch_size
    )
...
def beam_search(
  ...
  """
    ...
    def _inner_loop(
        i, alive_seq, alive_log_probs, finished_seq, finished_scores, finished_flags
    ):
    ...
    """
        ...
        alive_seq, alive_log_probs, _ = _grow_alive(
            topk_seq, topk_scores, topk_log_probs, topk_finished, batch_size, beam_size
        )
    ...
    """
        ...
        bound_is_met = tf.reduce_all(
            tf.greater(lowest_score_of_finished_in_finished, lower_bound_alive_scores)
        )
        ...
        if minimum_score:
            ...
            bound_is_met = tf.logical_or(
                bound_is_met,
                tf.reduce_all(tf.less(lower_bound_alive_scores, minimum_score_log)),
            )
