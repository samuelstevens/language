    ...
    def apply_gradients(self, grads_and_vars, global_step=None, name=None):
        ...
        for (grad, param) in grads_and_vars:
            ...
            if (
                ...
                and param_name in self.pretrained_param_names
            ):
            ...
            tf.summary.scalar(
                "updates/{}".format(param_name), tf.reduce_mean(update_with_lr)
            )
...
def create_optimizer(
    ...
    if freeze_pretrained_steps and restart_warmup_after_unfreeze:
        ...
        tf.summary.scalar(
            "is_pretraining", tf.cast(global_step < freeze_pretrained_steps, tf.int32)
        )
    else:
        learning_rate = tf.constant(value=init_lr, shape=[], dtype=tf.float32)

    tf.summary.scalar("global_step_count", global_steps_float)
    tf.summary.scalar("current_base_learning_rate", learning_rate)
    tf.summary.scalar("global_decay_step", current_step_in_decay)
    tf.summary.scalar("total_decay_steps", num_decay_steps)
    ...
    if num_warmup_steps:
        ...
        tf.summary.scalar("warmup_percent_done", warmup_percent_done)

        warmup_learning_rate = learning_rate * warmup_percent_done

        is_warmup = global_steps_int < warmup_steps_int

        tf.summary.scalar("is_warmup", tf.cast(is_warmup, tf.float32))
        learning_rate = tf.cond(
            is_warmup, lambda: warmup_learning_rate, lambda: learning_rate
        )
...
def build_train_op_with_pretraining(
    loss, model_config, pretrained_variable_names, use_tpu
):
    return create_optimizer(
        loss,
        model_config.training_options.optimizer_learning_rate,
        ...
        freeze_pretrained_steps=model_config.training_options.freeze_pretrained_steps,
        lr_after_restarting=model_config.training_options.after_restart_learning_rate,
    )
